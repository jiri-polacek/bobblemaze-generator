% evaluating puzzle complexity using simple theorem
%
function [ind]=index_simple(labyrinth, walls, bobblemaze_generator_complexity_parameters, find_solutions_complexity_parameters)
	...
	% positive & negative solved elements
	if nargin >= 3 && length(bobblemaze_generator_complexity_parameters) == 2
		pos=bobblemaze_generator_complexity_parameters(1);
		neg=bobblemaze_generator_complexity_parameters(2);
	else
		[pos, neg]=evaluate(teritory, maximus);
	end
	...
	% parameters from find_solutions function
	if nargin >= 4 && length(find_solutions_complexity_parameters) == 3
		before=find_solutions_complexity_parameters(1);
		after=find_solutions_complexity_parameters(2);
		rcounter=find_solutions_complexity_parameters(3);
	else
		[q, s, p, compl]=find_solutions(labyrinth, walls);
		before=compl(1);
		after=compl(2);
		rcounter=compl(3);
	end
	...
	rcounter_deduction=round(log2(rcounter/(before*after)+1));

	% compute index using simple theorem
	ind=pos+neg+solved_corners+corners-missing-rcounter_deduction;

	%pos, neg, solved_corners, corners, before, after, missing, rcounter_deduction
	%disp(['... extra hard .. 0 .. hard .. ', num2str(round(maximus/4)), ' .. moderate ..', num2str(round(maximus/2)), ' .. easy .. ', num2str(round(maximus/5*4)), ' .. very easy ...']);

endfunction

function [pos, neg]=evaluate(teritory, maximus)
	global  wallsmode walls_r walls_b SHIFT;
	V = []; X = []; Y = []; last = 0; neg = 0; pos = 0;
	[r,c] = size(teritory);
	do
		new = 0;
		for i=2:r-1
