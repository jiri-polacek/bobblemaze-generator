...
%
function [quantity, solutions, paths, complexity]=find_solutions(labyrinth, walls, verbose)
	global verb teritory stack dirs maximus mx my sol pth wallsmode walls_r walls_b SHIFT M rcounter;
	start_time = time;
	sol = {};
	pth = {};
	complexity = [0 0 0];
	...
	complexity(1)=sum(sum(teritory>0));
	if verb disp(['Valid numbers before presolving: ', num2str(complexity(1))]); end
	...
	complexity(2)=sum(sum(teritory>0));
	if verb disp(['Valid numbers after presolving: ', num2str(complexity(2))]); end
	
	rcounter = 0;
	sfind(sx, sy, 0);		% recursive searching
	solutions = sol;		% complete solutions
	quantity = length(sol);	% count of solutions
	paths = pth;			% paths vectors
	complexity(3)=rcounter;	% quantity of recursion calls

	if verb printf('\n'); disp(['Calculation time: ', num2str(time-start_time), ' s']); end
	if verb printf('\n'); disp(['Quantity of recursion calls: ', num2str(rcounter)]); end
...
function sfind(x, y, direction)
	global verb teritory stack dirs maximus mx my sol pth wallsmode walls_r walls_b M rcounter;
	rcounter++;
	if teritory(x,y) == maximus									% final number found
		...
		% simple progress bar
		if verb
			if mod(rcounter, 100) == 0
				printf('=')
			elseif length(stack) == maximus-5 
				printf('o')
